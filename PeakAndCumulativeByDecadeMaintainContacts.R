...
peakICU[Threshold+1]<-max(ICUdemandByTime);
totalICU[Threshold+1]<- sum(ICUriskByAge*(currentState[,3]+currentState[,2] )) 

}

plot(Thresholds*10, peakICU,col="red",
     main="Peak ICU demand",
     ylab="ICU demand",
     xlab="Quarantine Threshold",
     type="p",lwd=2, ylim=c(0,24000))

lines(Thresholds*10,Thresholds*0+520)


plot(Thresholds*10, totalICU,col="black",
     main="Total ICU demand",
     ylab="ICU demand",
     xlab="Quarantine Threshold",
     type="p",lwd=2,ylim=c(0,90000))
